...
def chat_page():
    ...
    if "waiting_llm" not in st.session_state:
        st.session_state["waiting_llm"] = False

    user_input = st.chat_input("Ask me something about Fluig!")

    if user_input and not st.session_state["waiting_llm"]:
        st.session_state["waiting_llm"] = True
        st.chat_message("human").markdown(user_input)

        with st.spinner("Thinking..."):
            try:
                answer = run_answer(
                    user_input, vector_store=vector_store, memory=memory
                )
